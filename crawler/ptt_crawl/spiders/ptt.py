...
class PttSpiderByPage(scrapy.Spider):
    ...
    def parse_post(self, resp):
        ...
        # get title
        try:
            self.item['title'] = resp.xpath('//meta[@property="og:title"]/@content').get()
        except IndexError:
            self.item['title'] = 'err'
        ...
        # get author, date
        try:
            data = sel.css('.article-meta-value::text').getall()  # [0]: author, [1]: board, [2]: title [3]: date
            self.item['author'] = data[0].split()[0]
            self.item['date'] = datetime.strptime(data[3], '%a %b %d %H:%M:%S %Y')
        except IndexError:
            self.item['author'] = 'err'
            self.item['date'] = datetime.strptime('Mon Jan 1 00:00:00 1980', '%a %b %d %H:%M:%S %Y')
